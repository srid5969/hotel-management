...
export const checkAdminRole = (asyncRouteHandler: ExpressMiddleware) => {
  return async (
    req: Request,
    res: Response,
    next: NextFunction
    ...
    try {
      ...
      const getUserRoleId = await userRepo.getUserRoleId(rolesConfig.ADMIN);
      if (!getUserRoleId) {
      ...
      if (checkUserRole.length === 0) {
        const noResult = new UnauthorizedResponse(res, {
          ...
          message: 'You do not have the permission to access this.',
        });
        return noResult.send();
      }
